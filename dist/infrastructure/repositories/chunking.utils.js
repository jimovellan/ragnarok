...
export function buildChunkTexts(knowledge, maxChars, overlapChars) {
    const header = [knowledge.title, knowledge.summary].filter(Boolean).join("\n");
    return chunkText(knowledge.content, maxChars, overlapChars).map((chunk) => header ? `${header}\n${chunk}` : chunk);
}
//# sourceMappingURL=chunking.utils.js.map