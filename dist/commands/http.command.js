 ...
 */
export function registerHttpCommand(program, repository, docRepository, docChunkRepository) {
    program
        .command('http')
        ...
        app.get('/api/knowledge/:key', async (req, res) => {
            const knowledge = await repository.getByKey(req.params['key']);
            if (!knowledge) {
        ...
        app.patch('/api/knowledge/:key', async (req, res) => {
            ...
            const knowledge = await repository.updateByKey(req.params['key'], changes);
            res.json(knowledge);
        });
        app.delete('/api/knowledge/:key', async (req, res) => {
            await repository.removeByKey(req.params['key']);
            res.status(204).send();
        });
        ...
        app.get('/api/docs/:key', async (req, res) => {
            const doc = await docRepository.getByKey(req.params['key']);
            if (!doc) {
        ...
        app.patch('/api/docs/:key', async (req, res) => {
            const changes = (req.body ?? {});
            const doc = await docRepository.updateByKey(req.params['key'], changes);
            res.json(doc);
        });
        app.delete('/api/docs/:key', async (req, res) => {
            await docRepository.removeByKey(req.params['key']);
            res.status(204).send();
        });
