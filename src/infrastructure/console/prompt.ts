...
function withRawKeypresses(onKeypress: (str: string, key: readline.Key) => void): () => void {
...
function handleExit(key: readline.Key, cleanup: () => void): void {
...
export function selectPrompt<T>(message: string, choices: SelectChoice<T>[]): Promise<PromptResult<T>> {
...
export function textPrompt(message: string, initialValue = ""): Promise<PromptResult<string>> {
  return new Promise((resolve) => {
...
export function viewScreen(lines: string[]): Promise<void> {
  return new Promise((resolve) => {
    console.clear();
